...
\begin{frame}{Tweak 2 --- ``communication recency'' (transmit the most recently learned belief) }
  ...
  \end{block}
  \vspace{2pt}
 Intuitively, learning inside real organisation happens by probing and \textit{testing}, rather than only by random discovery. Moreover, it makes sense that recent information circulates faster: that's why they're called ``news'' afterall.
  \vspace{4pt}
\end{frame}


\begin{frame}{Effect on information quality}
 Share of held positives that are stale:
  \begin{center}\small
  \begin{tabular}{@{}lcc@{}}
  \toprule
